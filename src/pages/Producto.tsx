    ...
    <Layout>
      <div className="container py-10">
        <nav className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground mb-8">
          <Link to="/" className="link-underline">{t("collection.home")}</Link> <span className="mx-2 opacity-50">/</span>
          ...
          <div>
            {product.stock <= 5 && (
              <div className="inline-flex items-center gap-2 bg-destructive/10 text-destructive px-3 py-1.5 text-[11px] uppercase tracking-[0.2em] mb-5">
                <Flame className="h-3.5 w-3.5" /> {t("product.onlyLeft", { n: product.stock })}
              </div>
            )}
            ...
            <div className="flex items-baseline gap-4 mt-6">
              ...
              <span className="text-xs text-muted-foreground">{t("product.ivaIncluded")}</span>
            </div>
            ...
            <div className="mt-8">
              <p className="text-[11px] uppercase tracking-[0.22em] mb-3">
                {t("product.finish")} <span className="text-muted-foreground normal-case tracking-normal">{swatchMeta[swatch].label}</span>
              </p>
            ...
            <div className="flex flex-wrap items-stretch gap-3 mt-8">
              <div className="flex items-center border border-foreground">
                <button className="px-4" onClick={() => setQty(Math.max(1, qty - 1))} aria-label={t("product.less")}>−</button>
                <span className="px-4 tabular-nums">{qty}</span>
                <button className="px-4" onClick={() => setQty(qty + 1)} aria-label={t("product.more")}>+</button>
              </div>
              <button onClick={() => add(product, swatch, qty)} className="btn-primary flex-1 min-w-[200px]">
                {t("product.addToCart")} · {(product.price * qty).toFixed(2)}€
              </button>
            </div>
            ...
            <div className="mt-8 grid grid-cols-3 gap-3 text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
              <div className="flex items-center gap-2"><Truck className="h-3.5 w-3.5 text-primary" /> {t("product.shipDays")}</div>
              <div className="flex items-center gap-2"><RotateCcw className="h-3.5 w-3.5 text-primary" /> {t("product.returnDays")}</div>
              <div className="flex items-center gap-2"><ShieldCheck className="h-3.5 w-3.5 text-primary" /> {t("product.secure")}</div>
            </div>

            {/* Accordion */}
            <Accordion type="single" collapsible className="mt-10 border-t border-border">
              <AccordionItem value="materiales">
                <AccordionTrigger className="text-[11px] uppercase tracking-[0.22em] hover:no-underline">{t("product.materials")}</AccordionTrigger>
                <AccordionContent className="text-sm text-muted-foreground leading-relaxed">{product.materials}{t("product.materialsExtra")}</AccordionContent>
              </AccordionItem>
              <AccordionItem value="medidas">
                <AccordionTrigger className="text-[11px] uppercase tracking-[0.22em] hover:no-underline">{t("product.measures")}</AccordionTrigger>
                <AccordionContent className="text-sm text-muted-foreground">{product.measurements}</AccordionContent>
              </AccordionItem>
              <AccordionItem value="envio">
                <AccordionTrigger className="text-[11px] uppercase tracking-[0.22em] hover:no-underline">{t("product.shipReturns")}</AccordionTrigger>
                <AccordionContent className="text-sm text-muted-foreground">{t("product.shipDesc")}</AccordionContent>
              </AccordionItem>
            </Accordion>
          </div>
        </div>
        ...
        <section className="mt-28">
          <div className="flex items-end justify-between mb-10">
            <h2 className="font-display text-3xl md:text-4xl">{t("product.alsoLike1")} <em className="italic">{t("product.alsoLikeEm")}</em></h2>
            <Link to={`/coleccion/${product.category}`} className="text-[11px] uppercase tracking-[0.22em] link-underline">{t("product.viewMore")}</Link>
          </div>
